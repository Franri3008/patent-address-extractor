...
echo "      --port 8000 --gpu-memory-utilization 0.45 \\"
echo "      --max-num-batched-tokens 32768 --max-num-seqs 8 \\"
echo "      --no-enable-prefix-caching --mm-processor-cache-gb 2"
echo ""
echo "  # LLM server (Gemma for text classification, port 8001):"
echo "  vllm serve google/gemma-3-4b-it \\"
echo "      --port 8001 --served-model-name gemma \\"
echo "      --gpu-memory-utilization 0.45 --max-num-seqs 16 \\"
echo "      --max-model-len 4096 --dtype auto"
echo ""
echo "  Then set config.json: llm.provider='vllm', llm.base_url='http://localhost:8001/v1', llm.model='gemma'"
